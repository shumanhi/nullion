...
"$NULLION_EXE" --port $NULLION_WEB_PORT >> "$logFile" 2>> "$errFile"
...
"$NULLION_TELEGRAM_EXE" --checkpoint "$NULLION_DIR\runtime-store.json" --env-file "$NULLION_ENV_FILE" >> "$telegramLogFile" 2>> "$telegramErrFile"
...
"$NULLION_SLACK_EXE" --checkpoint "$NULLION_DIR\runtime-store.json" --env-file "$NULLION_ENV_FILE" >> "$slackLogFile" 2>> "$slackErrFile"
...
"$NULLION_DISCORD_EXE" --checkpoint "$NULLION_DIR\runtime-store.json" --env-file "$NULLION_ENV_FILE" >> "$discordLogFile" 2>> "$discordErrFile"
...
if (-not $AUTOSTART_CONFIGURED) {
    ...
    Write-Host ""
    Write-Host "    $NULLION_EXE --port $NULLION_WEB_PORT" -ForegroundColor Cyan
    if ($TELEGRAM_ENABLED) {
        Write-Host ""
        Write-Info "Telegram was configured. Start it manually with:"
        Write-Host ""
        Write-Host "    nullion-telegram --checkpoint $NULLION_INSTALL_DIR\runtime-store.json --env-file $NULLION_ENV_FILE" -ForegroundColor Cyan
    }
    if ($SLACK_ENABLED) {
        Write-Host ""
        Write-Info "Slack was configured. Start it manually with:"
        Write-Host ""
        Write-Host "    nullion-slack --checkpoint $NULLION_INSTALL_DIR\runtime-store.json --env-file $NULLION_ENV_FILE" -ForegroundColor Cyan
    }
    if ($DISCORD_ENABLED) {
        Write-Host ""
        Write-Info "Discord was configured. Start it manually with:"
        Write-Host ""
        Write-Host "    nullion-discord --checkpoint $NULLION_INSTALL_DIR\runtime-store.json --env-file $NULLION_ENV_FILE" -ForegroundColor Cyan
    }
    Write-Host ""
}
...
if (Confirm-Prompt "Open Nullion in your browser now?") {
    ...
    $proc = Start-Process `
        ...
        -ArgumentList "--port", $NULLION_WEB_PORT `
